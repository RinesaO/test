...
const MSHLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      if (result.user?.role === 'msh') {
        navigate('/msh-dashboard');
      } else {
        setError(t('msh.accessDenied'));
      }
    } else {
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              >
                {loading ? t('auth.signingIn') : t('msh.loginButton')}
              </button>
            </div>
          </form>
        </div>
